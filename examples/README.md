...
| [`architecture-explainer.json`](../fixtures/specs/architecture-explainer.json) | A4, 4pp | `rentos` | **The reference for `lesson` mode.** A six-rung ladder in `meta`, an `analogy` before any mechanism, a `progressive` adding one part at a time, a `misconception` carrying the tension, and the glossary kept deliberately last. Read the ladder first. Uses `analogy`, `progressive`, `misconception`, `bridge`, `stack`, `process`, `swimlane`, `checklist`, `definitions`. |
